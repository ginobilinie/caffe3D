...
////////////////// Mod by Roger////////////////////////////////
  ...
  num_ = unary_terms->shape(0);//num();
  channels_ = unary_terms->shape(1);//channels();
  depth_ = unary_terms->shape(2);
  height_ = unary_terms->shape(3);//height();
  width_ = unary_terms->shape(4);//width();
...
/////////////////////////////////////////////////////////////////////
  if (this->blobs_.size() > 0) {
    ...
    blobs_[0].reset(new Blob<Dtype>(filters_shape_)); // spatial kernel weight
    blobs_[1].reset(new Blob<Dtype>(filters_shape_)); // bilateral kernel weight
    blobs_[2].reset(new Blob<Dtype>(filters_shape_)); // compatibility transform matrix
  }

  pairwise_.Reshape(data_shape_);
  spatial_out_blob_.Reshape(data_shape_);
  bilateral_out_blob_.Reshape(data_shape_);
  message_passing_.Reshape(data_shape_);
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
